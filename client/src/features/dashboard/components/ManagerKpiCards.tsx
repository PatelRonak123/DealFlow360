...
import { ArrowDown, ArrowUp, Award, ClipboardCheck, IndianRupee, Percent } from 'lucide-react';

interface ManagerKpiCardsProps {
  pendingCount: number;
}

export const ManagerKpiCards: React.FC<ManagerKpiCardsProps> = ({ pendingCount }) => {
  return (
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <div className="rounded-2xl border border-[#e7ebf7] bg-white p-5 shadow-[0_8px_24px_rgba(64,86,145,0.07)] transition hover:shadow-md">
        <div className="flex items-start justify-between">
          ...
          <span className="text-2xl font-bold tracking-tight text-[#17213a]">₹ 48.65 L</span>
        </div>
        ...
        <div className="mt-2 flex items-center justify-between">
          <p className="flex items-center gap-1 text-xs font-bold text-[#35ad79]">
            <ArrowUp size={13} /> 14.8% vs last month
          </p>
          <span className="text-[11px] text-[#8491aa]">38 open deals</span>
        </div>
      </div>

      {/* KPI 2: Pending Manager Approvals (High Privilege) */}
      <div className="relative overflow-hidden rounded-2xl border border-amber-200 bg-gradient-to-br from-white to-amber-50/50 p-5 shadow-[0_8px_24px_rgba(245,158,11,0.1)] transition hover:shadow-md">
        ...
        <div className="mt-2 flex items-center justify-between">
          <p className="text-xs font-bold text-amber-700">Manager override required</p>
          <span className="rounded bg-amber-100 px-1.5 py-0.5 text-[10px] font-bold text-amber-800">Priority</span>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-[#e7ebf7] bg-white p-5 shadow-[0_8px_24px_rgba(64,86,145,0.07)] transition hover:shadow-md">
        <div className="flex items-start justify-between">
          ...
          <span className="text-2xl font-bold tracking-tight text-[#17213a]">78%</span>
        </div>
        <div className="mt-3">
          <div className="flex items-center justify-between text-xs">
            <span className="font-semibold text-[#59657d]">Team Target Attainment</span>
            <span className="font-bold text-[#17213a]">₹ 62.4 L / ₹ 80 L</span>
          </div>
          <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-gray-100">
            <div className="h-full rounded-full bg-[#35ad79] transition-all duration-500" style={{ width: '78%' }} />
          </div>
        </div>
        <p className="mt-2 text-xs font-medium text-[#8491aa]">Ahead of monthly projection</p>
      </div>

      {/* KPI 4: Avg Team Discount Rate (Discount Governance) */}
      <div className="rounded-2xl border border-[#e7ebf7] bg-white p-5 shadow-[0_8px_24px_rgba(64,86,145,0.07)] transition hover:shadow-md">
        <div className="flex items-start justify-between">
          <span className="flex h-11 w-11 items-center justify-center rounded-full bg-indigo-50 text-indigo-600">
            <Percent size={20} />
          </span>
          <span className="text-2xl font-bold tracking-tight text-[#17213a]">13.4%</span>
        </div>
        <p className="mt-4 text-sm font-semibold text-[#59657d]">Avg. Team Discount</p>
        <div className="mt-2 flex items-center justify-between">
          <p className="flex items-center gap-1 text-xs font-bold text-[#35ad79]">
            <ArrowDown size={13} /> 1.2% discipline
          </p>
